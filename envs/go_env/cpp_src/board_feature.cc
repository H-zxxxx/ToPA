...
// 获取双方棋子(重载)
bool getStones(const Board* board, int* data) {
    // a 19x19 tensor
    memset(data, 0, MAX_COORD * sizeof(int));
    FOR_EACH_COORD(c)
        data[c] = board->infos[c].color;
    END_FOR
    return true;
}

// 获取颜色平面
bool getPlayerPlains(const Board* board, Stone player, float* data) {
    // a 19x19 tensor
    memset(data, 0, 2 * MAX_COORD * sizeof(float));
    FOR_EACH_COORD(c)
        if (player == BLACK) {
            data[c] = 1;
        }
...
// 用一个平面描述当前玩家, 黑子全1, 白子全0
bool getColor(const Board* board, Stone player, float* data) {
    // a 19x19 tensor
    memset(data, 0, MAX_COORD * sizeof(float));
    if (player == BLACK) {
        FOR_EACH_COORD(c) data[c] = 1;
        END_FOR
    }
}
...
#undef OPPONENT_LIB
#undef BOTH_HISTORY
#undef OUR_KO_AND_SUICIDE
#undef OUR_TRUE_EYE
#undef OUR_BLOCK_LIVES
#undef BOTH_HISTORY1
#undef BOTH_HISTORY2
#undef OUR_KO_AND_SUICIDE1
#undef OUR_TRUE_EYE1
#undef OPP_TRUE_EYE1
#undef OUR_BLOCK_LIVES1
#undef OPP_BLOCK_LIVES1



#define OUR_STONES 0
#define OPP_STONES 1
#define BOTH_HISTORY1 2
#define BOTH_HISTORY2 3
#define PLAYER_PLAINS 4

#define OUR_LIB 6
#define OPPONENT_LIB 9
#define OUR_KO_AND_SUICIDE 12
#define OUR_TRUE_EYE 13
#define OPP_TRUE_EYE 14
#define OUR_BLOCK_LIVES 15
#define OPP_BLOCK_LIVES 16

void encode17(const Board* board, float* features) {
    memset(features, 0, 17 * MAX_COORD * sizeof(float));
    
    Stone player = board->next_player;
    getStones(board, player, LAYER(OUR_STONES));
    getStones(board, OPPONENT(player), LAYER(OPP_STONES));
    getHistoryMap1(board, EMPTY, LAYER(BOTH_HISTORY1));
    getHistoryMap2(board, EMPTY, LAYER(BOTH_HISTORY2));
    getPlayerPlains(board, player, LAYER(PLAYER_PLAINS));
    ...
    getTrueEyeMap(board, player, LAYER(OUR_TRUE_EYE));
    getTrueEyeMap(board, OPPONENT(player), LAYER(OPP_TRUE_EYE));
    getBlockLives(board, player, LAYER(OUR_BLOCK_LIVES));
    getBlockLives(board, OPPONENT(player), LAYER(OPP_BLOCK_LIVES));
