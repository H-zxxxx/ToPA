...
#define __MACRO_MAX_BOARD_SIZE 19
#define __MACRO_MAX_BLOCK 256
...
extern int BOARD_SIZE;          // 实际棋盘尺寸, 可变
extern Coord MAX_COORD;         // 实际最大坐标数, 可变

namespace GoComm {  // 放在名称空间GoComm里

const int UPPER_BOARD_SIZE = __MACRO_MAX_BOARD_SIZE;  // 最大棋盘尺寸
const Coord UPPER_MAX_COORD = UPPER_BOARD_SIZE * UPPER_BOARD_SIZE;  // 最大坐标数
...
const BlockId MAX_BLOCK = __MACRO_MAX_BLOCK;  // 最大连通块数
...
// 快速坐标转换
#define XY_TO_COORD(x, y)  ((y) * BOARD_SIZE + (x))
#define COORD_TO_X(c)      ((c) % BOARD_SIZE)
#define COORD_TO_Y(c)      ((c) / BOARD_SIZE)
#define ON_BOARD(c)         (0 <= (c) && (c) < MAX_COORD)
#define ON_BOARD_XY(x, y)  (0 <= (x) && (x) < BOARD_SIZE && \
                            0 <= (y) && (y) < BOARD_SIZE)
