...
bool Init(int board_size, int history_dim, int encode_dim, int max_step, float komi) {
    ...
    BOARD_SIZE = board_size;  // 设置棋盘大小
    MAX_COORD = board_size * board_size;  // 设置最大坐标数
    ...
    for (int i = 0; i < history_dim_; ++i) {
        if (encode_dim_ == 9)  // 己方1,2,3及以上气连通块, 对方1,2,3及以上气连通块, 上一个历史落子点, 非法落子, 己方真眼
            GoFeature::encode9(&state->_boards[i], 
                encode_state + i * encode_dim_ * MAX_COORD);
...
int getLegalAction(const State* state, int* actions) {
    ...
    // 加上Pass==361
    actions[legal.num_moves] = MAX_COORD;
...
int getLegalNoEye(const State* state, int* actions) {
    ...
    // 加上Pass==361
    actions[candidate.num_moves] = MAX_COORD;
...
void getSumedEffectionDomain(const State* state, float* sum_ed) {
    memset(sum_ed, 0, MAX_COORD * sizeof(float));
